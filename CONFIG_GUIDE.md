...
1. 找到并编辑 `claude_desktop_config.json` 文件
2. 添加上述配置内容
3. 保存文件
4. 重启 Claude Desktop
...
在 Claude Desktop 中，你应该能看到 "data-analysis" 服务器已连接。
...
- **端口被占用**：修改 `main.py` 中的端口号（默认 3000）
...
如果需要从其他机器访问，修改配置中的 `localhost` 为服务器 IP 地址：
